...
from shared_utils import list_input_files, convert_yyyydoy_to_date, write_single_tile_xrdataset
from preprocess import construct_file_dict, create_single_tile_dataset, make_sorted_raster_stack, parse_date
...
class UnitTest(unittest.TestCase):
    ...
    def test_make_sorted_raster_stack(self):
        ...
        dates = [
            convert_yyyydoy_to_date(parse_date(x)) for x in file_dict_h5["h11v02"]["CGF_NDSI_Snow_Cover"]
        ]
        dates.sort()
        yyyydoy_strings = [d.strftime("%Y") + d.strftime("%j") for d in dates]
    ...
    def test_dask_h5_stack(self):
        ...
        start_time = time.time()
        h5_stack_da = make_sorted_h5_stack(fps_h5, yyyydoy_strings, variable_path, lazy=True)
